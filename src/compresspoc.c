/* compresspoc.c - MRT compression proof of concept
 *
 * This is a throwaway program based on bgp-explain used to determine
 * whether MRT files can be usefully compressed by de-duplicating
 * repeat announcements using a hash file
 */
...
struct OPTIONS {
  ...
  int quiet;
  int count;
  char *prefix; // path to the files before the hash suffixes
  char *tmp; // path to /tmp before the hash file name
};
...
// HASHTEST stuff
if (record->numerrors==0)
{
  uint8_t *buffer, r;
  uint32_t buflen;
  struct NLRI_LIST *r4, *r6, *u4, *u6;
  uint8_t *serialized_update;
  uint32_t updatelen;
  off_t hashposition;
  struct FILE_ENTRY ent = {};
  uint8_t entry_flag;

  serialized_update = serialize_bgp_update(record, &updatelen);
  buffer = serialize_nlri(m,&buflen);
  assert (serialized_update && buffer);
  r = deserialize_nlri(buffer, buflen, &r4, &r6, &u4, &u6);
  assert(r);

printf ("Serialized NLRIs: %u bytes, update %u bytes\n", buflen, updatelen);
  ent.length = htonl(buflen + sizeof(struct FILE_ENTRY) - sizeof(uint64_t));
  ent.timestamp_seconds = record->seconds;
  ent.microsecond_timestamp = record->microseconds;
  hashposition = hash_save_data(h, serialized_update, updatelen);
  if (hashposition<0) fprintf(stderr,"hashposition=%ld, %u bytes\n", 
    hashposition, updatelen);
  assert(hashposition >= 0);
  ent.position = (uint64_t) hashposition;
  switch (ent.bytes[0]) {
    case 0xff:
    case 0xfe: // the substitute starts with an escape byte, so escape it
      entry_flag = 0xfe;
      write(entriesfile, &entry_flag, sizeof(uint8_t));
    default:
      break;
  };
  write(entriesfile, &ent, sizeof(ent)); // length and position
  write(entriesfile, &buffer, buflen); // serialed nlris
  ...
  if (u4) free(u4);
  if (u6) free(u6);
  free(serialized_update);
} else {
fprintf (stderr,"Rawwrite this one because: %u errors\n", record->numerrors);
  uint8_t unmodifiedflag = 0xff;
  write(entriesfile, &unmodifiedflag, sizeof(uint8_t));
  write(entriesfile, record->mrt, (ssize_t) ntohl(record->mrt->length));
  h->h.rawsaves ++;
}
  return;
}
...
, int entriesfile
  ...
  while ((record = mrt_read_record(file))) {
    ...
    mrt_print_decode_message (record, length, bytes_read, options, stats, h,
      entriesfile);
    if (options->trace && options->badonly && record->trace_read 
        && record->numerrors) {
      fprintf (stdout, "  Read MRT record at byte %lu size %lu:\n",
...
int main (int argc, char **argv) {
  int i;
  struct HASH_FILE *hashfile;
  int entriesfile;
  char hash_file_name[4096];
  char uniques_file_name[4096];
  ...
  struct argp_option argp_options[] = {
    ...
    {"count",    'c', 0,      0,  
      ...
      "MRT entry.", 0 },
    {"prefix",   'p', "prefix",      0,  
      "path and file name prefix for output data files.", 0 },
    {"tmp",      'm', "tmp",      0,  
      "additional path prefix for hash files.", 0 },
    { 0 }
  };
  char args_doc[] = "";
  char argp_doc[] = 
  ...
  snprintf (hash_file_name, 4095, "%s%s.hash", options.tmp, options.prefix);
  snprintf (uniques_file_name, 4095, "%s.deduped", options.prefix);
  snprintf (data_file_name, 4095, "%s.data", options.prefix);

  hashfile = open_hash_file(hash_file_name, uniques_file_name);
  assert(hashfile!=NULL);
  entriesfile = open(data_file_name, O_RDWR | O_CREAT, 0644);
  assert(entriesfile>0);
  assert(lseek(entriesfile, 0, SEEK_END) >= 0);
  readandprintmrtfile(NULL, &options, &stats, hashfile, entriesfile);
  fprintf (stderr,"Hash stats: %u/%u buckets used, %u maximum depth\n"
    "  stored record size %u to %u bytes\n"
    "  %u unique of %u saved records + %u raw\n",
    hashfile->h.hash_buckets_used, 
    hashfile->h.hash_buckets, hashfile->h.hash_max_depth, 
    hashfile->h.minsize, hashfile->h.maxsize,
    hashfile->h.unique, hashfile->h.total, hashfile->h.rawsaves);
  close_hash_file(hashfile);
  close(entriesfile);
  ...
  fprintf(stderr,"Number of (error / unknown / total) MRT records: %i / %i / %i\n",
